...
export const BudgetWizard: React.FC<WizardProps> = ({ userId, onComplete }) => {
    ...
    // Statistics for headers - Ensuring strict number arithmetic..
    const totalRevenue = useMemo(() => products.reduce((sum, p) => sum + (Number(p.price) * Number(p.avg_monthly_qty)), 0), [products]);
    const totalCOGS = useMemo(() => fixedExpenses.filter(e => e.group === 'cogs').reduce((sum, e) => sum + Number(e.monthly_amount || 0), 0), [fixedExpenses]);
    const totalGA = useMemo(() => fixedExpenses.filter(e => e.group === 'ga').reduce((sum, e) => sum + Number(e.monthly_amount || 0), 0), [fixedExpenses]);
    ...
    const loadStepData = async () => {
        ...
        try {
            if (currentStep === 1) {
                ...
                // Sanitize data: Ensure monthly_amount is stored as a number, not a string from API
                const cleanData = data.map(item => ({
                    ...item,
                    monthly_amount: Number(item.monthly_amount)
                }));
                setFixedExpenses(cleanData);
            }
        } catch (err) {
            console.error(err);
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-center">
                    ...
                    <span className="text-2xl font-bold text-green-600">
                        {products.length > 0 ?
                            Math.round(((totalRevenue - products.reduce((sum, p) => sum + (Number(p.unit_cost) * Number(p.avg_monthly_qty)), 0)) / totalRevenue) * 100) + '%'
                            : '0%'}
                    </span>
                </div>
            </div>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            ...
                            {products.map(p => (
                                <tr key={p.id} className="group hover:bg-indigo-50/30 transition-colors">
                                    ...
                                    <td className="p-4 text-left font-mono">₪{Number(p.price).toLocaleString()}</td>
                                    <td className="p-4 text-left font-mono">{Number(p.avg_monthly_qty).toLocaleString()}</td>
                                    <td className="p-4 text-left font-mono text-gray-500">₪{Number(p.unit_cost).toLocaleString()}</td>
                                    <td className="p-4 text-left font-mono font-bold text-indigo-600">₪{(Number(p.price) * Number(p.avg_monthly_qty)).toLocaleString()}</td>
